 ...
 * <LI>创建者登录信息【{@link #getCreatedOpLog()}】：系统登录日志KEY【{@link ILogLoginEntity#getCode()}】，表示添加此记录到数据表中的登录信息是什么(如哪个IP、什么浏览器、哪个登录帐号等)？
 ...
 * <LI>最近修改者登录信息【{@link #getCreatedOpLog()}】：系统登录日志KEY【{@link ILogLoginEntity#getCode()}】，表示最近修改此记录的登录信息是什么(如哪个IP、什么浏览器、哪个登录帐号等)？
 ...
 * <LI>数据KEY【{@link #getCode()}】：数据的KEY，数据的“逻辑主键”，或与其他字段（如租户KEY、其他主表KEY等）组合后作为数据的“逻辑主键”。
 ...
 * <LI>将数据转换为JSON【{@link #toJsonString()}】：将实体数据转换成JSON格式。
 * </OL>
 * 
 * @author Ji Yongshan
 * @preserve all
