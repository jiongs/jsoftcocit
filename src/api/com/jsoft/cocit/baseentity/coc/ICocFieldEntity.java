 ...
 * @preserve all
 */
public interface ICocFieldEntity extends INamedEntity {
	/**
	 * 实体定义：逻辑外键，关联到“{@link ICocEntity#getCode()}”字段。
	 * <p>
	 * 用于描述该字段属于哪个实体（实体类、实体表）？
	 * 
	 * @return
	 */
	String getCocEntityCode();

	/**
	 * 字段分组：逻辑外键，关联到“{@link ICocGroupEntity#getCode()}”字段。
	 * <p>
	 * 用于描述该字段属于哪个分组？注：字段和分组必须属于同一个实体。
	 * 
	 * @return
	 */
	String getCocGroupCode();

	/**
	 * 字段数据类型：可选值参见“{@link ICocFieldEntity#DATA_TYPE_XXXXX}”值。
	 * 
	 * @return
	 */
	int getFieldType();
	 ...
	 * @return
	 */
	String getManyTargetEntityCode();

	/**
	 * 外键关联的目标实体：逻辑外键，关联到“{@link ICocEntity#getCode()}”字段。
	 * <p>
	 * 用来描述该字段引用到哪个模块（哪张表）？
	 * 
	 * @return
	 */
	String getFkTargetEntityCode();
