...
public abstract class BaseFieldView implements UIFieldView {
	...
	public void render(Writer out, UIFieldModel fieldModel, Object entityObject, String fieldName, Object fieldValue) throws Exception {
		...
		ICocFieldInfo entityFieldService = ((UIField) fieldModel).getFieldService();
		...
		if (entityFieldService.getFkTargetFieldCode() != null) {
			targetIdField = entityFieldService.getFkTargetFieldCode();
		}
		String targetTextField = "";

		boolean isFkOneField = entityFieldService.isManyToOne();
		ICocFieldInfo nameField = entityFieldService.getEntity().getFkNameField(entityFieldService.getFieldName());
		if (nameField != null) {
			targetTextField = nameField.getFkTargetFieldCode();
			...
			if (isFkOneField) {
				...
				if (targetIdField.equals(targetTextField)) {
					...
					if (nameField != null) {
						...
						ICocEntityInfo targetModule = entityFieldService.getFkTargetEntity();
		...
		case FieldModes.M:
			...
			if (StringUtil.hasContent(str)) {
				...
				write(out, " id=\"%s\" name=\"%s\" %s ", //
						fieldModel.getId(), //
						idFieldName, //
						styleText == null ? "" : ("style=\"" + styleText + "\"")//
				);
			}
