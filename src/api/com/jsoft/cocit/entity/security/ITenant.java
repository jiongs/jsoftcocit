 ...
 * <OL>
 * <LI>系统KEY【{@link #getSystemKey()}】：逻辑外键，关联到【{@link ISystem#getKey()}】字段。 COC平台包含了多套系统，该字段用来描述企业用户（系统租户）使用的是哪套系统？如果同一企业用户（系统租户）同时使用了多套系统，则用“|”分隔（如Sytem1|System2）。
 * <LI>数据源KEY【{@link #getDataSourceKey()}】:逻辑外键，关联到【{@link IDataSource#getKey()}】字段。每套系统可以使用不同的数据源，同理每个租户也可以使用不同的数据源。该字段即用来描述企业用户（系统租户）使用的是哪个数据源？
