 ...
 * 
 */
public interface ICommonConfig extends IConfig {

	public static final String CONFIG_FILE_PACKAGE = ICommonConfig.class.getPackage().getName().replace(".", "/") + "/";

	public static final String COC_CONFIG_FILE = "cocconfig";

	/**
	 * 平台支持的上传文件后缀
	 * <p>
	 * 如：upload.filter= 7z|aiff|asf|avi|bmp|csv|doc|fla|flv|gif|gz|gzip|jpeg|jpg |mid|mov|mp3|mp4|mpc |mpeg|mpg|ods|odt|pdf|png|ppt|pxd|qt|ram|rar|rm|rmi|rmvb |rtf|sdc|sitd|swf| sxc|sxw|tar|tgz|tif|tiff|txt|vsd|wav|wma|wmv|xls|xml|zip
	 */
	 ...
	 * NSL配置文件 配置项KEY
	 */
	static final String CONFIG_I18N = "config.i18n";
	 ...
	 * <LI>默认返回：com.jsoft.cocit.config.db.properties
	 * <LI>加载配置文件：系统首先加载默认配置文件；然后继续加载配置目录 {@link #getConfigClassDir()} 或 {@link #getConfigDir()} 下的同名的配置文件。 在配置目录下查找配置文件时使用的文件名不带任何文件夹部分。
	 * <LI>如： {@link #CONFIG_DB}=com.jsoft.cocit.config.db.properties 或 {@link #CONFIG_DB} =com.jsoft.cocit.config.db
	 * </UL>
	 * 
	 * @return
	 */
	 ...
	 * <LI>默认返回：com.jsoft.cocit.config.rootuser.properties
	 * <LI>加载配置文件：系统首先加载默认配置文件；然后继续加载配置目录 {@link #getConfigClassDir()} 或 {@link #getConfigDir()} 下的同名的配置文件。 在配置目录下查找配置文件时使用的文件名不带任何文件夹部分。
	 * <LI>如：{@link #CONFIG_USER} =com.jsoft.cocit.config.rootuser.properties 或 {@link #CONFIG_USER} =com.jsoft.cocit.config.rootuser
	 * </UL>
	 * 
	 * @return
	 */
	 ...
	 * <LI>默认返回：com.jsoft.cocit.config.ioc.js
	 * <LI>加载配置文件：系统首先加载默认配置文件；然后继续加载配置目录 {@link #getConfigClassDir()} 或 {@link #getConfigDir()} 下的同名的配置文件。 在配置目录下查找配置文件时使用的文件名不带任何文件夹部分。
	 * <LI>如：{@link #CONFIG_IOC}=com.jsoft.cocit.config.ioc.js 或 {@link #CONFIG_IOC}=com.jsoft.cocit.config.ioc.json 或 {@link #CONFIG_IOC}=com.jsoft.cocit.config.ioc
	 * </UL>
	 * 
	 * @return
	 */
	 ...
	 * <b>说明：</b>
	 * <UL>
	 * <LI>配置项KEY： {@link #CONFIG_I18N}
	 * <LI>默认后缀： .properties
	 * <LI>默认返回：com.jsoft.cocit.config.nls.properties
	 * <LI>加载配置文件：系统首先加载默认配置文件；然后继续加载配置目录 {@link #getConfigClassDir()} 或 {@link #getConfigDir()} 下的同名的配置文件。 在配置目录下查找配置文件时使用的文件名不带任何文件夹部分。
	 * <LI>如： {@link #CONFIG_I18N}=com.jsoft.cocit.config.nls.properties 或 {@link #CONFIG_I18N}=com.jsoft.cocit.config.nls
	 * </UL>
	 * 
	 * @return
	 */
	String getI18NConfig();
	 ...
	 * <UL>
	 * <LI>相对路径以'/'开头
	 * <LI>如果初始化【Cocit平台】上下文环境时指定了配置路径，则返回该配置路径；否则将获取配置项{@link #PATH_CONFIG} 中指定的路径；如果配置文件中没有{@link #PATH_CONFIG}配置项，则返回默认值。
	 * <LI>相对路径可以是SERVLET环境路径的子目录，其真实路径通过 {@link #getConfigDir()} 获取。
	 * <LI>相对路径也可以是SERVELT环境"/WEB-INF/classes"下的子目录，其真实路径通过 {@link #getConfigClassDir()} 获取。
	 * <LI>配置项KEY: {@link #PATH_CONFIG}
	 * <LI>默认返回：/config
	 * <LI>
	 * 如：{@link #PATH_CONFIG}=/config 则表示系统可以从SERVLET环境的/config目录或/WEB-INF/classes/config下加载相关配置文件。
	 * </UL>
	 * 
	 * @return
	 */
	 ...
	 * <LI>如：{@link #PATH_CONFIG}=/WEB-INF/config 【Cocit平台】程序安装在D:/Cocit/webapp目录下，则该方法将返回: D:/Cocit/webapp/WEB-INF/config
	 * </UL>
	 * 
	 * @return
	 */
	...
	// * {@link #getConfigPath()} 。
	// * <LI>为了避免配置文件可能会被客户端直接查看，因此【Cocit平台】将配置文件作为程序的一部分存放在类目录下。
	// * <LI>如：{@link #PATH_CONFIG}=/config
	// * 【Cocit平台】程序安装在D:/Cocit/webapp目录下，则该方法将返回:
	// * D:/Cocit/webapp/WEB-INF/classes/config
	// * </UL>
	// *
	// * @return
	// */
	 ...
	 * <LI>
	 * 环境路径下的日志文件可能会被客户端直接查看，因此除了需要兼容遗留系统外，一般不将日志文件直接存放在环境路径下，建议在日志路径配置中加上前缀/ WEB-INF
	 * <LI>如：{@link #PATH_LOGS}=/WEB-INF/logs 【Cocit平台】程序安装在D:/Cocit/webapp目录下，则该方法将返回: D:/Cocit/webapp/WEB-INF/logs
	 * </UL>
	 * 
	 * @return
	 */
	 ...
	 * <LI>
	 * 环境路径下的日志文件可能会被客户端直接查看，因此除了需要兼容遗留系统外，一般不将日志文件直接存放在环境路径下，建议在日志路径配置中加上前缀/ WEB-INF
	 * <LI>如：{@link #PATH_LOGS}=/WEB-INF/tmp 【Cocit平台】程序安装在D:/Cocit/webapp目录下，则该方法将返回: D:/Cocit/webapp/WEB-INF/tmp
	 * </UL>
	 * 
	 * @return
	 */
