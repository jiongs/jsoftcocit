...
public @interface CocColumn {
	...
	/**
	 * 外键字段所关联的目标实体，其值来自于【{@link CocEntity#code()}】。
	 * <p>
	...
	/**
	 * 用在外键字段上，用来描述冗余外键依赖关系，即该当前外键字段是哪个外键字段的冗余？通常和{@link #fkTargetField()}一起成对出现。
	 * <p>
	 * <B>应用场景：</B>orgKey是逻辑外键字段，是真正的外键；orgName是该逻辑外键对应的Display字段，是冗余字段。 <br>
	 * orgKey关联到目标实体的逻辑主键；而orgName则关联到目标实体的name字段。
	 * <p>
	 * <B>用法举例：</B><br>
	 * CocColumn(propName="orgKey", fkTargetEntity="hr_organization", fkTargetField="code") <br>
	 * CocColumn(propName="orgName", fkTargetEntity="hr_organization", fkTargetField="name", fkDependField="orgKey") <br>
	 * 
	 * @return
	 */
	...
	/**
	 * 用在外键字段上，指定关联的目标实体是否是该实体的父实体？即该模块是目标模块的子模块。
	 * <p>
	...
	/**
	 * 用在外键字段上，指定是否级联删除子表数据？
	 * <p>
	 * 当前字段是一个外键字段，外键字段引用了主表数据，如果主表数据被删除了，是否级联删除此表中的数据？
	 * <p>
	 * 
	 * @return
	 */
	public boolean fkCascadeDelete() default false;
	 ...
	 * <p>
	 * 语法：其值关联到（{@link IPatternAdapter#getName()}）
	 * <UL>
	 ...
	 * </UL>
	 * <UL>
	 * <LI>多个操作模式之间可以用“空格、逗号”分隔，操作编码来自{@link CocAction#code()}；
	 ...
	 * 默认值：自动值规则组成
	 * <p>
	 * 取变量值：$V{tenant.code},$V{user.name},$V{system.date}
	 * <p>
	 * 取其他外键字段值：$F{fkField.field1}
	 * 
	 * @return
	 */
