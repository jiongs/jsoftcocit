...
public @interface CocAction {
	...
	/**
	 * 操作码(即操作命令)：操作类型是由平台提供的，参见{@link OpCodes#OP_CODE_XXX}。
	 * <p>
	 * 同一个操作码可以对应多个不同的具体操作，通过{@link #code()}来区别。
	 * <p>
	 * 相同的操作码通常具有相同默认操作界面。
	 * 
	 * @return
	 ...
	 * <p>
	 * <UL>
	 * <LI>用途：用来指定执行操作时的个性化界面，UI页面文件可以是“.jsp”或“.st”文件，也可以不是具体的页面，而是通过{@link CuiForm}所定义的表单KEY({@link CuiForm#code()})；
	 ...
	 * @return
	 */
	boolean uiWindowHeader() default true;
	...
	/**
	 * 命令链-前拦截器：即执行该操作前，按顺序调用命令链中的“命令拦截器”的接口方法。多个拦截器之间用逗号(,)分隔。
	 * 
	 * @return
	 */
	...
	/**
	 * 命令链-后拦截器：即执行该操作后，按顺序调用命令链中的“命令拦截器”的接口方法。多个拦截器之间用逗号(,)分隔。
	 * 
	 * @return
	 */
