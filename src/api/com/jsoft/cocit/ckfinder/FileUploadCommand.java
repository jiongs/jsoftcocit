...
import com.jsoft.cocit.baseentity.common.IFileInfoEntityExt;
...
public class FileUploadCommand extends Command {
	 ...
	 *             when error occurs.
	 */
	protected void handleOnUploadCompleteCallFuncResponse(final OutputStream out, final String errorMsg, final String path) throws IOException {
		this.ckFinderFuncNum = this.ckFinderFuncNum.replaceAll("[^\\d]", "");
		out.write(("window.parent.CKFinder.tools.callFunction(" + this.ckFinderFuncNum + ", '" + path + this.newFileName + "', '" + errorMsg + "');").getBytes("UTF-8"));
	}
	 ...
	 *             when error occurs
	 */
	protected void handleOnUploadCompleteResponse(final OutputStream out, final String errorMsg) throws IOException {
		out.write("window.parent.OnUploadCompleted(".getBytes("UTF-8"));
		out.write(("\'" + this.newFileName + "\'").getBytes("UTF-8"));
		out.write((", \'" + (this.errorCode != Constants.Errors.CKFINDER_CONNECTOR_ERROR_NONE ? errorMsg : "") + "\'").getBytes("UTF-8"));
	 ...
	 */
	@Override
	public void initParams(final HttpServletRequest request, final IConfiguration configuration, final Object... params) throws ConnectorException {
	...
	private boolean uploadFile(final HttpServletRequest request) {
		if (!AccessControlUtil.getInstance(configuration).checkFolderACL(this.type, this.currentFolder, this.userRole, AccessControlUtil.CKFINDER_CONNECTOR_ACL_FILE_UPLOAD)) {
	...
	private void saveToDB(FileItem item, String folder) {
		IFileInfoEntityExt info = (IFileInfoEntityExt) Mirror.me(EntityTypes.FileInfo).born();
		IOrm orm = Cocit.me().orm();

		info.setContentType(item.getContentType());
		info.setName(item.getName());
		info.setContentLength(item.getSize());
		String filePath = folder + "/" + this.newFileName;
		info.setPath(filePath);
		info.setCode(DateUtil.format(new Date(), "yyyyMMddHHmmss") + "-" + Integer.toHexString(filePath.hashCode()));
		info.setNamePinyin(StringUtil.toPinyinFirstChar(info.getName()));
	...
	private boolean validateUploadItem(final FileItem item, final String path) {
		...
		if (!FileUtils.checkFileName(this.newFileName) || FileUtils.checkIfFileIsHidden(this.newFileName, configuration)) {
			this.errorCode = Constants.Errors.CKFINDER_CONNECTOR_ERROR_INVALID_NAME;
			return false;
		}

		int checkFileExt = FileUtils.checkFileExtension(this.newFileName, configuration.getTypes().get(type), configuration, true);
		if (checkFileExt == 1) {
		...
		try {
			...
			if (!FileUtils.checkFileSize(configuration.getTypes().get(this.type), item.getSize()) && !(configuration.checkSizeAfterScaling() && ImageUtils.isImage(file))) {
				this.errorCode = Constants.Errors.CKFINDER_CONNECTOR_ERROR_UPLOADED_TOO_BIG;
				return false;
			}

			if (configuration.getSecureImageUploads() && ImageUtils.isImage(file) && !ImageUtils.checkImageFile(item)) {
				this.errorCode = Constants.Errors.CKFINDER_CONNECTOR_ERROR_UPLOADED_CORRUPT;
				return false;
			}
	...
	protected void getCurrentFolderParam(final HttpServletRequest request) {
		...
		if (currFolder == null || currFolder.equals("") || currFolder.equals("/")) {
			this.currentFolder = "/" + DateUtil.format(new Date(), "yyyyMM") + "/";
		} else {
