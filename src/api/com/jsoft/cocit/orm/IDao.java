...
public interface IDao {
	...
	/**
	 * 与 update(String tableName, Chain chain, Condition condition) 一样，不过，数据表名，将取自 POJO 的数据表声明，请参看 '@Table' 注解的详细说明
	 * 
	 * @param classOfT
	 *            实体类型
	 * @param chain
	 ...
	 * 
	 * @param tableName
	 *            表名 - 格式为 <b>tableName[:idName]</b> 比如 ： <b>t_pet</b> 或者 <b>t_pet:id</b> 尤其在 SqlServer2005 的环境下，需要用 t_pet:id 的形式来指明 ID 字段，否则 不能分页
	 * @param condition
	 *            条件 - <b style=color:red>请注意：</b> 你传入的 Condition 实现必须考虑到 没有 'Entity<?>' 传入。即 toSql 函数的参数永远为 null。
	 * @param pager
	 *            翻页信息
	...
	/**
	 * Fetch one object from DB, it is upon the insert sequence and DB implementation about which one will be fetched.
	 * 
	 * @param classOfT
	 *            The POJO java type
