...
public class Admin0Action extends BaseAdminAction {
	...
	public synchronized UIModel clearConfigCache() {
		...
		try {
			...
			cocit.getDataManagerFactory().release();
			cocit.getEntityEngine().release();
			cocit.getEntityServiceFactory().release();
			cocit.getSecurityEngine().release();
			((IExtOrm) cocit.orm()).clearMapping();
	...
	public synchronized UIModel setupCocitFromPackage() {
		...
		try {
			IDataModelEngine moduleEngine = Cocit.me().getEntityEngine();

			// if (Cocit.me().getConfig().isProductMode()) {
			moduleEngine.setupCocitDBTables();
	...
	public synchronized UIModel exportToJson(@Param("cocentity") String entityKeys) {
		Cocit coc = Cocit.me();
		ILoginSession login = coc.getHttpContext().getLoginSession();
		IEntityInfoFactory entityServiceFactory = coc.getEntityServiceFactory();
		...
		try {
			String tenantKey = login.getTenant().getCode();
			String folder = coc.getContextDir() + "/WEB-INF/tmp/" + DateUtil.getNowDate();
			IDataModelEngine entityEngine = coc.getEntityEngine();

			List<String> keyList = StringUtil.toList(entityKeys);
			for (String key : keyList) {
				ICocEntityInfo cocEntity = entityServiceFactory.getEntity(key);
