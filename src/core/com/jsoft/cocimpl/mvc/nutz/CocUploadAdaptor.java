 ...
 * <LI>如：public Map upload(@Param("upload") TempFile tmpfile, String moduleId, String fieldId)
 * </UL>
 * 
 */
public class CocUploadAdaptor extends AbstractAdaptor {

	private UploadingContext context;

	public CocUploadAdaptor() throws IOException {
		context = new UploadingContext(Cocit.me().getConfig().getTempDir());
	}

	public CocUploadAdaptor(UploadingContext context) {
		this.context = context;
	}

	public CocUploadAdaptor(String path) {
		context = new UploadingContext(path);
	}

	public CocUploadAdaptor(String path, int buffer) {
		this(path);
		context.setBufferSize(buffer);
	}

	public CocUploadAdaptor(String path, int buffer, String charset) {
		this(path);
		context.setBufferSize(buffer);
		context.setCharset(charset);
	}

	public CocUploadAdaptor(String path, int buffer, String charset, int poolSize) {
		context = new UploadingContext(new NutFilePool(path, poolSize));
		context.setBufferSize(buffer);
		context.setCharset(charset);
	}

	public CocUploadAdaptor(String path, int buffer, String charset, int poolSize, int maxFileSize) {
		context = new UploadingContext(new NutFilePool(path, poolSize));
		context.setBufferSize(buffer);
		context.setCharset(charset);
