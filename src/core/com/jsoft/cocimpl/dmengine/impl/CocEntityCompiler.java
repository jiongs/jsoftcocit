...
public class CocEntityCompiler {
	...
	private IDataModelEngine engine;

	CocEntityCompiler(IDataModelEngine engine) {
		this.engine = engine;
	}
	...
	private String checkSrc(ICocEntity system) {
		if (system.isDisabled())
			return system.getName() + "(" + system.getCode() + "," + system.getId() + ")实体模块被停用;";
	...
	private String checkSrc(ICocEntity system, ICocFieldEntity field) {
		String prop = engine.getPropName(field);
		String name = field.getName() + "(" + prop + ")";
		if (field.isDisabled())
			return name + "字段被停用;";
		if (!StringUtil.isBlank(field.getFkTargetFieldCode()))
			return name + "是引用字段;";
	 ...
	 * @return 源代码片段
	 */
	private String makePackageCode(ICocEntityInfo system) {
	 ...
	 *            实体模块
	 */
	private String makeClassAnnotation(ICocEntityInfo module) {
		...
		src.append(makeProp("code", module.getCode(), ""));
		src.append(makeProp("catalog", module.getCatalogCode(), ""));
		...
		src.append("\n, actions = {");
		src.append("\n// begin action");
		List<ICocActionInfo> actions = module.getActions(null);
		if (actions != null) {
			int count = 0;
			for (ICocActionInfo action : actions) {
				...
				src.append(makeProp("code", action.getCode(), ""));
		...
		src.append("\n, groups = {");
		src.append("\n// begin group");
		List<ICocGroupInfo> groups = module.getGroups();
		if (groups != null) {
			int count = 0;
			for (ICocGroupInfo group : groups) {
				 ...
				 * 字段组信息注解
				 */
				src.append(String.format("@%s(name = \"%s\", key = \"%s\", fields = {", CocGroup.class.getSimpleName(), group.getName(), group.getCode()));
				src.append("\n\t\t\t\t// begin column");
				...
				if (groups != null) {
					...
					for (ICocFieldInfo field : fields) {
						...
						src.append(makeProp("fkTargetField", field.getFkTargetFieldCode(), ""));
						src.append(makeProp("fkTargetEntity", field.getFkTargetEntityCode(), ""));
	...
	private String makeClassCode(ICocEntityInfo module) {
		StringBuffer src = new StringBuffer();
	...
	private String getComplireVersion(ICocEntityInfo module) {
		String version = DateUtil.formatDateTime(new Date());
		((ICocEntityExt) module.getEntityData()).setCompileVersion(version);
	 ...
	 */
	@SuppressWarnings("unused")
	private String genSrcOfTargetAnnotation(ICocFieldEntity fld) {
		StringBuffer src = new StringBuffer();
	 ...
	 * @return 源代码片段
	 */
	private String makeFieldAnnotation(ICocFieldEntity fld) {
		StringBuffer src = new StringBuffer();
		...
		// if (!StringUtil.isEmpty(fld.getCode())) {
		// src.append(", fieldName = \"").append(fld.getCode()).append("\"");
		// }
		...
		// }
		// if (!StringUtil.isEmpty(fld.getRefFieldCode())) {
		// src.append(", refColumn = \"").append(fld.getRefFieldCode()).append("\"");
		// }
		// if (!StringUtil.isEmpty(fld.getRefModuleCode())) {
		// src.append(", refEntity = \"").append(fld.getRefModuleCode()).append("\"");
		// }
	...
	private String makeModuleJavaCode(StringBuffer src, ICocEntityInfo module) {
		...
		// 检查字段
		List<ICocFieldInfo> checkedFields = new LinkedList();
		List<? extends ICocFieldInfo> fields = module.getFieldsOfEnabled();
		if (fields != null && fields.size() > 0) {
			...
			for (ICocFieldInfo fld : fields) {
				...
				if (StringUtil.isBlank(prop) || !var.matcher(prop).find())
					...
					refFlds.add(info);
				// else if (!StringUtil.isEmpty(fld.getRefModuleCode()))
		...
		// 生成属性声明语句
		for (ICocFieldEntity fld : checkedFields) {
	 ...
	 *             编译业务实体类出错将抛出编译错误异常
	 */
	synchronized List<String> compileSystem(ICocEntityInfo module, boolean copyToClassesDir) throws CocException {
		if (LogUtil.isInfoEnabled()) {
			LogUtil.info("编译<%s(%s,%s)>实体模块......", module.getName(), module.getCode(), module.getId());
		}

		List<ICocEntityInfo> collectedModules = new LinkedList();
		collectedModules.add(module);

		if (LogUtil.isInfoEnabled()) {
			LogUtil.info("编译<%s(%s,%s)>实体模块: 计算需要编译的依赖实体模块...", module.getName(), module.getCode(), module.getId());
		}
		...
		if (LogUtil.isInfoEnabled()) {
			...
			for (ICocEntity mdl : collectedModules) {
				logBuf.append(mdl).append("(" + mdl.getCode() + "," + mdl.getId() + ")").append(mdl.getClassName()).append("\n");
			}
			LogUtil.info("编译<%s(%s,%s)>实体模块: 计算需要编译的依赖实体模块: 结束. 共<%s>个实体模块需编译[\n%s]", module.getName(), module.getCode(), module.getId(), collectedModules.size(), logBuf);
		}
		...
		try {
			...
			for (int i = collectedModules.size() - 1; i >= 0; i--) {
				ICocEntityInfo mdl = collectedModules.get(i);
			...
			String info;
			for (ICocEntityInfo mdl : collectedModules) {
				className = mdl.getClassName();
				srcCode = new StringBuffer();
				info = makeModuleJavaCode(srcCode, mdl);

				if (info.length() > 0)
					infos.add(mdl.getName() + "(" + mdl.getCode() + "," + mdl.getId() + ")：" + info);
	 ...
	 *            编译的目标业务实体模块
	 */
	private void collectBizSystems(List collectedModules, ICocEntityInfo targetModule) {
		if (!collectedModules.contains(targetModule)) {
			collectedModules.add(targetModule);
		}
		List<? extends ICocFieldEntity> fields = targetModule.getFkFields();
		for (ICocFieldEntity fld : fields) {
			if (!fld.isDisabled()) {
				ICocEntityInfo refModule = Cocit.me().getEntityServiceFactory().getEntity(fld.getFkTargetEntityCode());
				if (!collectedModules.contains(refModule) && !refModule.isDisabled()) {
					if (LogUtil.isDebugEnabled())
						LogUtil.debug("实体模块<%s(%s,%s)>字段<%s(%s,%s)>引用到实体模块<%s(%s,%s)>", targetModule.getName(), targetModule.getCode(), targetModule.getId(), fld.getName(), fld.getCode(), fld.getId(), refModule.getName(), refModule.getCode(),
								refModule.getId());
