...
public class RowActionsButtonCellView implements UICellView {

	public String getName() {
		return ViewNames.CELL_VIEW_ROWACTIONS_BUTTON;
	}
	...
	private void printButtons(Writer out, UIActionsModel model, Object dataObject, List<Node> nodes) throws IOException {
		...
		for (Node node : nodes) {
			...
			if (node.size() > 0) {
				...
				Serializable dataID = (Serializable) ObjectUtil.getId(dataObject);
				...
				if (!StringUtil.isBlank(urlExpr)) {
					...
					str = node.get("opUrl", "");
					if (!StringUtil.isBlank(str)) {
						WriteUtil.write(out, ", opUrl: '%s'", str);
						WriteUtil.write(out, ", rowId: '%s'", dataID);
					}
				}
