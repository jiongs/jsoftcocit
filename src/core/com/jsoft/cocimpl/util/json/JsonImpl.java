...
public class JsonImpl {
	 ...
	 */
	public JsonImpl(Writer writer, JsonFormat format) {
		this.format = format;
		this.writer = writer;
	...
	private static boolean isCompact(JsonImpl render) {
	...
	public void renderValue(Object obj) throws IOException {
		if (null == obj)
			...
			} else if (ClassUtil.isDate(obj.getClass())) {
				string2Json(DateUtil.formatDateTime((Date) obj));
	...
	public static String toJson(Object obj) {
		try {
			...
			new JsonImpl(writer, JsonFormat.nice()).render(obj);

			writer.flush();

			return sb.toString();
		} catch (IOException e) {
			throw ExceptionUtil.throwEx(e, JsonException.class);
		}
	}
		...
		try {
			if (json.trim().startsWith("[") && json.endsWith("]")) {
				...
				if (json.startsWith("/") || json.startsWith("\\")) {
					// 从上下文环境中读取文件
					path = Cocit.me().getContextDir() + json;
					is = Files.findFileAsStream(path);
				} else {
					// 从classes目录下读取文件
					path = Cocit.me().getConfig().getClassDir() + "/" + json;
					is = Files.findFileAsStream(path);
					// if (is == null) {// 从classes下的配置路径下读取文件
					// path = Cocit.me().getConfig().getClassDir() + "/" + Cocit.me().getConfig().getConfigPkg().replace(".", "/") + json;
					// is = Files.findFileAsStream(path);
					// }
					...
					// if (is == null) {
					// path = Cocit.me().getConfig().getConfigPkg().replace(".", "/") + json;
					// is = Files.findFileAsStream(path);
					// }
				...
				if (is == null && is2 == null)
					throw new CocException("对象配置文件不存在!");
			...
			if (json1 != null)
				array1 = (T[]) Json.fromJson(ClassUtil.forName(klass.getName() + "[]"), json1);
			if (json2 != null)
				array2 = (T[]) Json.fromJson(ClassUtil.forName(klass.getName() + "[]"), json2);

		} catch (Throwable e) {
			throw new CocException("从JSON加载对象出错! [json: %s] %s", json, ExceptionUtil.msg(e));
		} finally {
			if (is != null)
				try {
					is.close();
