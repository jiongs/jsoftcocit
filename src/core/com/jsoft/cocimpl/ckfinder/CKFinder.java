...
public class CKFinder {
	...
	public static void doUpload(final HttpServletRequest request, final HttpServletResponse response) {
		...
		try {
			...
			if (configuration.getEvents() != null) {
				if (configuration.getEvents().run(EventTypes.BeforeExecuteCommand, args, configuration)) {
					CommandHandlerEnum.valueOf(command.toUpperCase()).execute(request, response, configuration, Cocit.me().getServletContext());
				}
			} else {
				CommandHandlerEnum.valueOf(command.toUpperCase()).execute(request, response, configuration, Cocit.me().getServletContext());
			}
		...
		try {
			if (currentCommand != null) {
				...
				if (command instanceof XMLCommand) {
					CommandHandlerEnum.XMLERROR.execute(request, response, configuration, Cocit.me().getServletContext(), e);
				} else {
					CommandHandlerEnum.ERROR.execute(request, response, configuration, Cocit.me().getServletContext(), e);
				}
			} else {
				CommandHandlerEnum.XMLERROR.execute(request, response, configuration, Cocit.me().getServletContext(), e);
			}
	...
	private enum CommandHandlerEnum {
		 ...
		 *             when error occurs
		 */
		private void execute(final HttpServletRequest request, final HttpServletResponse response, final IConfiguration configuration, final ServletContext sc, final Object... params) throws ConnectorException {
			Command com = null;
			try {
