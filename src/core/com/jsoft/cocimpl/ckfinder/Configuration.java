...
public class Configuration implements IConfiguration {
	...
	public static Configuration getInstance() throws Exception {
		if (instance == null) {
			...
			instance.init();
			AccessControlUtil.getInstance(instance).loadACLConfig();
			ConfigurationFactory.getInstace().setConfiguration(instance);
			ErrorUtils.setLangDir(Cocit.me().getServletContext());
		}
	...
	public Configuration() {
		this.xmlFilePath = "/config/ckfinder-config.xml";
	...
	public void init() throws Exception {
		clearConfiguration();
		this.loading = true;
		File file = new File(FileUtils.getFullPath(Cocit.me().getConfig().getWebInfoDir() + xmlFilePath));
	 ...
	 * 
	 * @param value
	 *            userPathBuilderImpl configuration value
	 */
	...
	private void setACLs(final NodeList childNodes) {
		...
		for (int i = 0; i < childNodes.getLength(); i++) {
			...
			if (childNode.getNodeName().equals("accessControl")) {
				...
				// COC: 角色支持逗号分隔
				if (acl != null) {
					String[] roles = StringUtil.toArray(acl.getRole(), ",");
					if (roles.length > 1) {
						for (String role : roles) {
							AccessControlLevel a = new AccessControlLevel(acl);
							a.setRole(role);
							this.accessControlLevels.add(a);
						}
					} else {
	 ...
	 */
	private ResourceType createTypeFromXml(final String typeName, final NodeList childNodes) {
	 ...
	 * @throws Exception
	 *             when error occurs during reading config file
	 */
	 ...
	 * 
	 * @param plugins2
	 *            new configuration plugins list.
	 */
	 ...
	 * 
	 * @param accessControlLevels2
	 *            new configuration ACLs list.
	 */
	 ...
	 * 
	 * @param types2
	 *            new configuration resource types list.
	 */
