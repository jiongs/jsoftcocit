var r20 = /%20/g, rbracket = /\[\]$/, rCRLF = /\r?\n/g, rsubmitterTypes = /^(?:submit|button|image|reset|file)$/i, rsubmittable = /^(?:input|select|textarea|keygen)/i, rcheckableType = (/^(?:checkbox|radio)$/i);
...
function serialize($form) {
	return jQuery.param(serializeArray($form));
};
...
function serializeArray($form) {
	return getElementsOf($form).map(function(i, elem) {
		var val = $(this).val();
...
function getElementsOf($form) {
	return $form.map(function() {
		var elements = $.prop($form, "elements");
		if (elements) {
...
function checkHeartbeat_coc() {
	$.ajax({
		type : 'POST',
		url : jCocit.contextPath + "/coc/chkHeartbeat/" + new Date().getTime(),
		async : true,
		data : "",
		dataType : "json",
