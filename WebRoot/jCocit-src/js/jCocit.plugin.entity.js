(function($, jCocit) {
	var contextPath = jCocit.contextPath, dialogWidth = 600, dialogHeight = 400;
	...
	jCocit.entity = {
		...
		doCloseWindow : function(btn, token) {
			if (window.opener) {
				...
				if (dialog.length > 0) {
					...
					jCocit.AdminConsole.closeTab(btn);
				}
			}
		},
		...
		editGridRow : function(btn) {
			...
			if (options.editUrl) {
				...
				} else if (options.editRowIndex != rowIndex) {
					$.messager.showMsg(jCocit.entity.defaults.promptSave);
				}
			}
		},
