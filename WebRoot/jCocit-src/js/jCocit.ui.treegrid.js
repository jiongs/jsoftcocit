...
(function($) {
	...
	function loadData(gridTable, id, treeNodes, append) {
		...
		var state = $d(gridTable, "treegrid");
		...
		$(gridTable).datagrid("setSelectionState");
		...
		render : function(gridTable, $body, frozen) {
			...
			function _makeHtml(frozen, treeLevel, treeNodes) {
				...
				for ( var i = 0; i < treeNodes.length; i++) {
					opts.renderRowIndex = opts.renderRowIndex + 1;
					var row = treeNodes[i];
					...
					var cls = (opts.renderRowIndex % 2 && opts.striped) ? 'class="datagrid-row datagrid-row-alt' : 'class="datagrid-row';
					if (row.checked) {
						cls += " datagrid-row-checked";
					}
					cls += '"';
					html.push('<tr id="' + rowId + '" ' + cls + ' node-id=' + row[opts.idField] + ' ' + styleAttr + '>');
					...
					if (col.checkbox) {
						...
						if (field == opts.treeField) {
							...
							if (row.state == "closed") {
								...
								if (row.checked) {
									html.push('<span class="TrC TrC1"></span>');
								} else {
									html.push('<span class="TrC"></span>');
								}
	...
	$.fn.treegrid.defaults = $.extend({}, $.fn.datagrid.defaults, {
		...
		finder : {
			...
			getRow : function(gridTable, p) {
				...
				var ret = $(gridTable).treegrid("find", id);
				return ret;
			},
			getRows : function(gridTable) {
				return $(gridTable).treegrid("getChildren");
			}
		},
		/**
		 * arg: row , queryParams-
