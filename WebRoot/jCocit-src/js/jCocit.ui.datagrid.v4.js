(function($, jCocit) {
	...
	function _init(_50, _51) {
			...
			$(_50).children("thead").each(function() {
				...
				$(this).find("tr").each(function() {
					...
					$(this).find("th").each(function() {
						var th = $(this);
						var col = $.extend({}, jCocit.parseOptions(this, [ "field", "align", "halign", "order", "width", {
							sortable : "boolean",
							checkbox : "boolean",
							resizable : "boolean",
							fixed : "boolean"
		...
		var _56 = $(
				"<div class=\"datagrid-wrap\">" + "<div class=\"datagrid-view\">" + "<div class=\"datagrid-view1\">" + "<div class=\"datagrid-header\">"
						...
						+ "<div class=\"datagrid-footer-inner\"></div>" + "</div>" + "</div>" + "</div>" + "</div>").insertAfter(_50);
		_56.panel({
			doSize : false,
			cls : "datagrid"
		});
	...
	function _5a(_5b) {
				...
				for ( var j = 0; j < _6d.length; j++) {
					...
					if (col.checkbox) {
						...
						if (col.field) {
							...
							var pos = _indexOfRow(_6b, col.field);
							if (pos >= 0) {
							...
							if (col.width) {
								var _70 = jCocit.parseValue("width", col.width, dc.view, _5d.scrollbarSize);
		...
		function _62() {
			var _71 = [];
			var _72 = getColumnFields(_5b, true).concat(getColumnFields(_5b));
			for ( var i = 0; i < _72.length; i++) {
				var col = getColumnOption(_5b, _72[i]);
				if (col && !col.checkbox) {
	...
	function _75(_76) {
		...
		_7b.unbind(".datagrid").bind("click.datagrid", function(e) {
			...
			if (_7d) {
				...
				var col = getColumnOption(_76, _7e);
				if (col.resizable == false) {
					return;
				}
		...
		_7b.each(function() {
			$(this).resizable({
				...
				onStopResize : function(e) {
					...
					var col = getColumnOption(_76, _80);
	...
	function _a2(_a3, _a4) {
		...
		if (typeof _a4 == "string") {
			var _aa = _a4;
			var col = getColumnOption(_a3, _aa);
			if (!col.sortable || _a5.resizing) {
				return;
			}
			var _ab = col.order || "asc";
			var pos = _indexOfRow(_a8, _aa);
			if (pos >= 0) {
		...
		for ( var i = 0; i < _a8.length; i++) {
			var col = getColumnOption(_a3, _a8[i]);
			_ad.find("div." + col.cellClass).addClass("datagrid-sort-" + _a9[i]);
		}
		if (_a6.remoteSort) {
			_ae(_a3);
		} else {
			loadData(_a3, $(_a3).datagrid("getData"));
		}
	...
	function _b0(_b1) {
		...
		function _b6() {
			...
			var _b7 = 0;
			var cc = [];
			var _b8 = getColumnFields(_b1, false);
			for ( var i = 0; i < _b8.length; i++) {
				var col = getColumnOption(_b1, _b8[i]);
				if (_b9(col)) {
					_b7 += col.width;
					cc.push({
						field : col.field,
		...
		function _b5() {
			var _be = false;
			var _bf = getColumnFields(_b1, true).concat(getColumnFields(_b1, false));
			$.map(_bf, function(_c0) {
				var col = getColumnOption(_b1, _c0);
				if (String(col.width || "").indexOf("%") >= 0) {
					var _c1 = jCocit.parseValue("width", col.width, dc.view, _b3.scrollbarSize) - col.deltaWidth;
					if (_c1 > 0) {
						col.boxWidth = _c1;
						_be = true;
					}
	...
	function _c2(_c3, _c4) {
		...
		if (_c4) {
			...
			var _c7 = false;
			var _c8 = getColumnFields(_c3, true).concat(getColumnFields(_c3, false));
			for ( var i = 0; i < _c8.length; i++) {
				var _c4 = _c8[i];
				var col = getColumnOption(_c3, _c4);
				if (col.auto) {
					resize(_c4);
					_c7 = true;
				}
	...
	function _d1(_d2, _d3) {
		...
		if (_d3) {
			...
			var ff = getColumnFields(_d2, true).concat(getColumnFields(_d2, false));
		...
		_d8(_d2);
		function fix(_d9) {
			var col = getColumnOption(_d2, _d9);
			if (col.cellClass) {
	...
	function _d7(_da) {
		...
		dc.view.find("td.datagrid-td-merged").each(function() {
			...
			var col = getColumnOption(_da, td.attr("field"));
			var _dc = col.boxWidth + col.deltaWidth - 1;
			for ( var i = 1; i < _db; i++) {
				td = td.next();
				col = getColumnOption(_da, td.attr("field"));
	...
	;
	function getColumnOption(_e0, _e1) {
		function _e2(_e3) {
			if (_e3) {
	...
	;
	function getColumnFields(tableElement, frozenColumns) {
		var options = $.data(tableElement, "datagrid").options;
		var columns = (frozenColumns == true) ? (options.frozenColumns || [ [] ]) : options.columns;
		if (columns.length == 0) {
			return [];
		}
		var aa = [];
		...
		for ( var _eb = 0; _eb < columns.length; _eb++) {
			$.map(columns[_eb], function(col) {
				var _ec = _ed(aa[_eb]);
				if (_ec >= 0) {
					var _ee = col.field || "";
		...
		if (!_f3.remoteSort && _f3.sortName) {
			...
			_f1.rows.sort(function(r1, r2) {
				...
				for ( var i = 0; i < _f4.length; i++) {
					var sn = _f4[i];
					var so = _f5[i];
					var col = getColumnOption(_f0, sn);
	...
	;
	function selectRow(_115, _116, _117) {
		...
		if (opts.singleSelect) {
			unselectAll(_115, true);
			_118.selectedRows = [];
		}
		if (!_117 && opts.checkOnSelect) {
			checkRow(_115, _116, true);
		}
		if (opts.idField) {
	...
	;
	function unselectRow(_11a, _11b, _11c) {
	...
	;
	function unselectAll(_125, _126) {
	...
	;
	function checkRow(_12b, _12c, _12d) {
		...
		if (opts.singleSelect && opts.selectOnCheck) {
			uncheckAll(_12b, true);
			_12e.checkedRows = [];
		}
		if (!_12d && opts.selectOnCheck) {
			selectRow(_12b, _12c, true);
		}
	...
	;
	function uncheckRow(_12f, _130, _131) {
	...
	function _140(_141, _142, _143) {
		...
		if (!_143) {
			...
			if (_147) {
				if (_indexOfRow(_146, row) == -1) {
					if (_indexOfRow(_145, row) == -1) {
						_145.push(row);
					}
				}
			}
	...
	function _13d(_155, _156) {
		...
		tr.children("td").each(function() {
			...
			var col = getColumnOption(_155, _157);
			if (col && col.editor) {
				var _158, _159;
	...
	function _17b(_17c) {
		...
		function _184(ids, _185) {
			for ( var i = 0; i < ids.length; i++) {
				...
				if (_186 >= 0) {
					(_185 == "s" ? selectRow : checkRow)(_17c, _186, true);
				}
			}
		}
		;
	...
	$.fn.datagrid = function(options, args) {
		if (typeof options == "string") {
			return $.fn.datagrid.methods[options](this, args);
		}
		options = options || {};
		return this.each(function() {
			...
			if (state) {
				...
				var gridObj = _init(this, opts.rownumbers);
				if (!opts.columns) {
					opts.columns = gridObj.columns;
				}
				if (!opts.frozenColumns) {
					opts.frozenColumns = gridObj.frozenColumns;
				}
	...
	$.fn.datagrid.methods = {
		...
		getColumnFields : function(jq, _1c0) {
			return getColumnFields(jq[0], _1c0);
		},
		getColumnOption : function(jq, _1c1) {
			return getColumnOption(jq[0], _1c1);
		},
	...
	$.fn.datagrid.parseOptions = function(_1eb) {
		var t = $(_1eb);
		return $.extend({}, $.fn.panel.parseOptions(_1eb), jCocit.parseOptions(_1eb, [ "url", "toolbar", "idField", "sortName", "sortOrder", "pagePosition", "resizeHandle", {
	...
	$.fn.datagrid.parseData = function(_1ec) {
		...
		t.find("tbody tr").each(function() {
			data.total++;
			var row = {};
			$.extend(row, jCocit.parseOptions(this, [ "iconCls", "state" ]));
