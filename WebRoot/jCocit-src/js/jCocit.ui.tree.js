...
(function($, jCocit) {
			...
			function _doSequentialScroll() {
				...
				for (; i < $firstPages.length; i++) {
					...
					if (currPageIndex >= 0) {
						...
						// $log("ui.tree:scroll: firstPage={node:[{1}], top:[{2}], height:[{3}]}, currPageIndex=[{4}], prevHiddenSize=[{5}], currPageSize=[{6}],
						// nextHiddenSize=[{7}]", time0,
						// $firstPage.find(">li:first>div.TrN>span.TrT").text(), firstTop, firstHeight, currPageIndex, prevHiddenSize,
	...
	function checkNode(selfUL, nodeDIV, checked, cascadeCheck, ignoreOnCheck) {
		...
		if (cascadeCheck || (typeof cascadeCheck == "undefined" && opts.cascadeCheck)) {
	...
	function _callback(config, times, nodesSize) {
		...
		if (times != 0)
			$log("ui.tree:loadNodes: NO.{3} Loaded! tree=[{2}], folder=[{5}], keywords=[{4}], nodes=[{1}]", time0, nodesSize, selfUL.title || selfUL.id || "", times, opts.keywords || "",
					(folderNode ? $(folderNode.target).attr("title") : ""));
	...
	function _appendNodes2(config, dataRows) {
				...
				if (from < dataRows.length) {
					if (pageSize) {
						...
						if (token == state.token) {
							...
							$log("ui.tree:loadNodes:_makeRows 1: keywords=[{1}], nodeSize=[{2}], dataRows.length=[{3}]", time0, config.opts.keywords, nodeSize,
									dataRows.length);
						...
						_sto(function() {
							...
							if (token == state.token) {
								...
								$log("ui.tree:loadNodes:_makeRows 2: keywords=[{1}], nodeSize=[{2}], dataRows.length=[{3}]", time0, config.opts.keywords, nodeSize,
										dataRows.length);
		...
		function _makeRow(token, item, resultHTML, depth, itemCord, isLast, isRootFirst) {
			...
			nodeSize = 1, //
			children = item.childrenURL || item.children, // if original children is string, after loaded children is Array.
			asyn = $.type(children) == "string", // children is childrenURL means asyn load.
			...
			if (asyn) {
				if (!item.childrenURL) {
					...
					if (orgItem)
						orgItem.childrenURL = item.children;
					// else {
					// $log("ui.tree:loadNodes:_makeRow[" + item.id + "]: not found! childrenURL=[" + item.children + "] itemCord=[" + itemCord + "]");
					// }
				}
			 ...
			 * add node checkbox
			 */
			if (opts.checkbox && !item.uncheckable && (!opts.onlyLeafCheck || !children)) {
	...
	function expandNode(selfUL, nodeDIV, callback, ignoreOnExpand) {
		...
		// $log("ui.tree:expandNode: [" + node.id + "] loaded=[" + (loaded || "") + "], childrenURL=[" + (node.childrenURL || "") + "], children=[" + (($.type(children) != "string" &&
		// children) ?
		// children.length : 0) + "]");

		if ($.type(loaded) != "undefined" && loaded == "0" && node.childrenURL) {
	...
	function selectNode(selfUL, nodeDIV, ignoreOnSelect) {
		...
		if (opts.checkbox && opts.checkOnSelect) {
			...
			// var cascadeCheck = opts.onlyLeafValue;
			checkNode(selfUL, nodeDIV, true, opts.onlyLeafValue);
		}
	...
	function doKey(selfUL, key) {
		...
		case keyCode.RIGHT:
			if ($selectedNode.length) {
				...
				if (!$selectedNode.attr("childrenURL"))
					_findNextVisibleRowNode();
			} else
				_findNextVisibleRowNode();
	...
	$.fn.tree = function(options, args) {
		...
		return this.each(function() {
			...
			if (opts.data) {
				...
				$ac("TrL", $loading);
				_sto(loadNodes, 100, this, this, opts.data, false, function(times) {
					if (times == 2)
						$rc("TrL", $loading);
				});
			} else {
	...
	$.fn.tree.parseOptions = function(selfUL) {
		return $.extend({}, jCocit.parseOptions(selfUL, [ "width", "height", "childrenURL", "method", "styleName", "keywords", {
			checkbox : "b",
			checkLeftCls : "b",
			cascadeCheck : "b",
			onlyLeafCheck : "b",
	...
	$.fn.tree.defaults = {
		...
		loader : function(folderNode, queryData, onLoadSuccess, onLoadError) {
			...
			$log("ui.tree:loader: doAjax... dataURL=[" + dataURL + "]");
