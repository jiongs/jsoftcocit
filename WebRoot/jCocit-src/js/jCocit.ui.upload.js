...
SWFUpload.prototype.initSettings = function() {
	...
	this.ensureDefault("button_image_url", jCocit.contextPath+"/jCocit/css/images/blank.gif");
...
SWFUpload.prototype.getFlashVars = function() {
	...
	return [ "movieName=", encodeURIComponent(this.movieName), "&amp;uploadURL=", encodeURIComponent(this.settings.upload_url), "&amp;useQueryString=",
			encodeURIComponent(this.settings.use_query_string), "&amp;requeueOnError=", encodeURIComponent(this.settings.requeue_on_error), "&amp;httpSuccess=", encodeURIComponent(a),
			"&amp;assumeSuccessTimeout=", encodeURIComponent(this.settings.assume_success_timeout), "&amp;params=", encodeURIComponent(b), "&amp;filePostName=",
		...
		init : function(options, swfUploadOptions) {
			...
			return this.each(function() {
				...
				if (flashInstalled) {
					 ...
					 * COCIT: Overwrite.
					 */
					$combo = $('<div class="Cb"><div class="CbB"></div><div class="upload-text"><input type="text" value="" class="CbT" /><input type="hidden" value="" class="CbV" /></div></div>');
					$combo.css({
					...
					$(".CbT", $combo).css({
						height      : (settings.comboHeight-5) + 'px',
						width       : (settings.comboWidth - 24) + 'px',
					}).val(settings.text);
...
(function($, jCocit) {
	...
	$.fn.upload = function(options, args) {
		if (typeof options == "string") {
			return this.uploadify(options, args);
		}
		options = options || {};
		return this.each(function() {
			...
			if (!state) {
				...
				$(this).uploadify(state.options);
			}
		});
	};

	$.fn.upload.parseOptions = function(targetHTML) {
		return $.extend({}, $.parseOptions(targetHTML, [ "width", "height"]));
	};

	$.fn.upload.defaults = {
		buttonText : 'Browse',
		buttonImage : jCocit.contextPath+'/jCocit/css/images/ui_upload.gif',
		swf : jCocit.contextPath+'/jCocit/css/images/ui_upload_uploadify.swf',
		uploader : jCocit.contextPath+'/coc/upload',
